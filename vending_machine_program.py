...
def update_dispenser_container(user_drink):
	...
	"""
	print(
		f"\n===update_dispenser_container> BEFORE UPDATE drink_order: {user_drink}"
	...
	"""
	print(
		f"\n===update_dispenser_container> AFTER UPDATE drink_order: {user_drink}"
		f" coins_dispenser contains: \n{current_dispenser_resource}"
	)
	"""
	return
...
one_drink = True  # Initially the coins_dispenser is full so there is at least one drink_order to offer
...
while one_drink:
	...
	if user_choice in DRINK_SELECTION:
		...
		if current_drink_availability[selected_drink]:
			...
			if payment:
				print(f"\n Here is your ordered drink_order <{selected_drink}>")
				print(f"\n Here is your change <{change_back}>")
				print(f"\n Many thanks for using our services - Till soon!")
				update_dispenser_container(selected_drink)
			else:
				print(f"\n Insufficient payment for your ordered drink_order <{selected_drink}>")
				print(f"\n Payed {change_back} but your drink_order costs {dispenser_prices[selected_drink]}")
